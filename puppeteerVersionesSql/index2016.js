...
async function getDataFromWebPage() {
    ...
    const results = await page.evaluate(() => {
        const secondOxa = document.querySelectorAll('div.oxa')[4];
        let data = [];
...
let sqlScript = "--Insertar datos de versiones;\n\n";
    ...
    results.forEach(row => {
        ...
        BEGIN
            ...
            VALUES ('2016','${row.build}','${row.fileVersion}','${row.kbDescription}','${row.releaseDate}');
        END
        ELSE 
        BEGIN
            UPDATE versionesSql
        ...
        fs.writeFileSync('versionesSql2016.sql',sqlScript);
